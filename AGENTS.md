...
# Commit Messages & Automations

You MUST write descriptive, structured commit messages adhering to the Conventional Commits specification (e.g. `feat: ...`, `fix: ...`, `ci: ...`, `docs: ...`). Commit messages should clearly describe *what* changed and *why*, ensuring they are "best of breed" for tracking history.
When writing commit messages, you must account for automated GitHub Actions workflows (e.g., semantic versioning, promotions, releases) so that the correct semver tag (patch, minor, major) is automatically generated:
